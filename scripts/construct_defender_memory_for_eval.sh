...
is_true "${QA_EARLY_STOPPING:-}" && args+=(--qa-early-stopping)
[[ -n "${QA_STOP_QUESTION_COUNT:-}" ]] && args+=(--qa-stop-question-count "${QA_STOP_QUESTION_COUNT}")
[[ -n "${QA_STOP_ACCURACY:-}" ]] && args+=(--qa-stop-accuracy "${QA_STOP_ACCURACY}")
[[ -n "${QA_STOP_CHECK_INTERVAL:-}" ]] && args+=(--qa-stop-check-interval "${QA_STOP_CHECK_INTERVAL}")
